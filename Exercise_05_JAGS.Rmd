...
```{r,echo=FALSE}
library(rjags) # Had to install JAGS from website http://www.sourceforge.net/projects/mcmc-jags/files
library(coda)
```
...
```{r}
# Diagnose the MCMC convergence
plot(jags.out) # Plot sample time series and density plots

gelman.diag(jags.out) # Calculate variance proportions for BGR diagnosis
# They're close to 1 so all chains have converged, or it looks like it

BGR <- gelman.plot(jags.out)
# For S, the ratios are always below 1.05 which indicates early convergence. For the b parameters, after approcimately 500 iterations the variance ratio falls closer to 1, so we'll take a burn-in period of 500 iterations.
```

```{r}
# Diagnose MCMC result after removing burn-in
burnin = 500                                   ## determine convergence
jags.burn <- window(jags.out, start = burnin)  ## remove burn-in
plot(jags.burn) 
# The plots have no aparent change for me
```
```{r}
# Assess autocorrelation and effective sample size for the final analysis
acfplot(jags.burn)
# The autocorrelations are small, decreasing very fast after 1 lag already.
effectiveSize(jags.burn)
cumuplot(jags.burn, probs = c(0.025, 0.25, 0.5, 0.75, 0.975))
# The confidence intervals and interquartile range seem stable with increasing number of iterations included. We see that after around 13000 iterations (effective sample size for all the parameters), the CI estimation doesn't fluctuate anymore

```

```{r}
# Summary of parameter statistics
summary(jags.burn)

summary(lm(y~x1+x2+x1:x2))
# The parameter mean estimates are very similar to the results from the frequentist model

# Marginal parameter distribution plots
out <- as.matrix(jags.burn)
colnames(out) <- c('S', 'b[1]', 'b[2]', 'b[3]', 'b[4]')
apply(out,2,hist)

# Pair plots
pairs(out)
cor(out)
# Naturally, regression coefficients are highly correlated
```