---
title: "Exercise_02_Logistic.Rmd"
author: "Pepa"
date: "2022-08-09"
...
plot_trajectory(r = 1.95)
plot_trajectory(r=2.05)
plot_trajectory(r=2.5)
```
Comment: We see that because of the discrete nature of the simulation, when the reproduction rate is increased, the simulation oscilates close to the population capacity K. The models become increasingly chaotic.
...
plot(x,dunif(x),type='l')  		## that’s a lowercase “L” for “line”

plot(x, punif(x), type = 'l')

p = seq(0, 1, 0.01)
plot(p, qunif(p), type = 'l')

hist(runif(100),main=100,probability=TRUE,breaks=40)  
				#here breaks defines number of bins in the histogram
...
```{r}
x <- rlnorm(10000,meanlog=0,sdlog=0.5)
y <- sin(x)
hist(x,main="Original distribution",breaks=40)
abline(v=quantile(x,c(0.025,0.5,0.975)),lty=c(2,1,2),lwd=3,col="orange")
abline(v=mean(x),col="red",lwd=3,lty=3)
...
hist(KE, main='samples of K')
```

5. Plot a sample of 10 different trajectories (through time) from your ensemble (on one graph).
```{r}
selection = sample(1:NE, size=10)
plot(n[selection[1],], type='l', lwd=2)
for(i in 2:9){
  lines(n[selection[i],], type ='l', col=i,lwd=2)
}
```

6. Plot a histogram of your population forecast at time = 15.
```{r}
hist(n[,15], breaks=20,main='Histogram of forecast at t=15',xlab='N(t=15)')
```
...
```{r}
plot(1:NT, n.stats[2,], type = 'l', lwd=2, xlab='time',ylab='n')
ciEnvelope(1:NT, ylo=n.stats[1,], yhi=n.stats[3,],col=adjustcolor("lightgrey",alpha.f=0.5))
...
plot(1:NT, n.stats[2,], type = 'l', lwd=2, xlab='time',ylab='n',main=paste('r=', r))
ciEnvelope(1:NT, ylo=n.stats[1,], yhi=n.stats[3,],col=adjustcolor("lightgrey",alpha.f=0.5))
}
