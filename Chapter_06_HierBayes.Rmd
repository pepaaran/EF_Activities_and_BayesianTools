...
NOTE: Pepa uses BayesianTools as the package for fitting bayesian models rather than rjags. This is what the GECO lab primarily uses in its packages too.
...
set.seed(123) # Set random seed and package and R versions for reproducibility
sessionInfo()
```
...
```{r}
# Define log likelihood function for the density
likelihood <- function(param){
  # param[1] = mu  and  param[2] = sigma
  singlelikelihoods = dnorm(dat$density, mean = param[1], sd = param[2], log = T)
  return(sum(singlelikelihoods))
}

# Define posterior distributions
setUp <- createBayesianSetup(likelihood, lower = c(0,0), upper = c(10,10))

# Run MCMC and calculate marginal likelihood
settings <- list(iterations = 3000, message = F, nrChains = 3)
out <- runMCMC(bayesianSetup = setUp, sampler = "Metropolis", settings = settings)
M <- marginalLikelihood(out)
...
```{r}
# Plot posterior CI and PI
out.mat <- getSample(out) # obtain matrix of MCMC posterior samples
credInt <- getCredibleIntervals(out.mat, quantiles = c(0.025, 0.975))
predInt <- getPredictiveIntervals(out.mat, model = function(x){rnorm(1, x[1], x[2])})

ggplot(dat, aes(x=time, y=density, color=rep)) +
  ylim(6.5, 8.5) +
  geom_line() +
  geom_point() +
  geom_abline(slope = 0, intercept = MAP(out)$parametersMAP[1], lty = 3) +
  geom_abline(slope = 0, intercept = credInt[1,1]) +
  geom_abline(slope = 0, intercept = credInt[2,1]) +
  geom_abline(slope = 0, intercept = predInt$posteriorPredictiveCredibleInterval[1,1], lty = 2) +
  geom_abline(slope = 0, intercept = predInt$posteriorPredictiveCredibleInterval[2,1], lty = 2)
...
```{r}
dat$t <- as.integer(as.factor(dat$time))
```
...
```{r}
# Define log likelihood function for the density with time as a random effect
likelihood_t <- function(param){
  # param[1] = mu,  param[2] = sigma, param[3] = tau, param[4:13] = alpha_t
  mut <- param[1] + param[4:13]
  llObs <- dnorm(dat$density, mean = mut[dat$t], sd = param[2], log = T)
  llRand <- dnorm(param[4:13], mean = 0, sd = param[3], log = T)
  return(sum(llObs) + sum(llRand))
}

# Define posterior distributions
setUp_t <- createBayesianSetup(likelihood_t, lower = c(5,0.01,0.01,rep(-1, 10)), upper = c(10,10,10,rep(1, 10)))

# Run MCMC and calculate marginal likelihood
settings <- list(iterations = 3000, message = F, nrChains = 3)
out_t <- runMCMC(bayesianSetup = setUp_t, sampler = "Metropolis", settings = settings)
M_t <- marginalLikelihood(out_t)

# Report summary statistics
summary(out_t)
plot(out_t)
gelmanDiagnostics(out_t, plot = T)
correlationPlot(out_t)

# THIS GIVES WEIRD WARNINGS WHEN THE LIKELIHOOD IS ZERO AND LOG-LIKELIHOOD -Inf. Why?????
```
```{r}
# Plot posterior CI and PI for the random effect model
out_t.mat <- getSample(out_t) # obtain matrix of MCMC posterior samples
credInt_t <- getCredibleIntervals(out_t.mat, quantiles = c(0.025, 0.975))
predInt_t <- getPredictiveIntervals(out_t.mat, model = function(x){rnorm(1, x[1], x[2])})
