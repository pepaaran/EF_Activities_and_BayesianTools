---
title: "Exercise_05B_Regression.Rmd"
author: "Pepa"
date: "2022-08-15"
...
```{r}
plot(var.out)
# The MCMC seems to converge because the samples of all three chains are around the same value, fluctuating randomly. One can observe in the BGR plot that, particularly after 400 iterations, the ratio of the within-chain variance to the across-chain variance is below 1.05. 
gelman.diag(var.out)
# The ratio being one indicates that we have reached convergence

# Remove samples from before convergence appeared.
burnin = 400
...
```{r}
summary(var.burn)
# The estimated expected coefficients are close to the true ones

# Plot marginal distributions
var.burn.mat <- as.matrix(var.burn)
for(cn in colnames(var.burn.mat)){
  hist(var.burn.mat[,cn], main = paste("Histogram of", cn), breaks=20, freq=F,
       xlab=cn)
  lines(density(var.burn.mat[,cn]), col=2)
  abline(v=mean(var.burn.mat[,cn]), col=2, lwd=2)
}
# Both plots of beta look normal, while the precision histogram is slightly skewed to the lef
```
```{r}
cor(var.burn.mat)
pairs(var.burn.mat)
# The regression coefficients are highly correlated, but not with the precision. This is also visible on the pair plots. Furthermore, since the posterior distributions are centered, one can use the mean parameters from the marginal distributions to get the MAP estimates.

```
```{r}
summary(var.burn)
summary(lm(y~x1))
# Both models coincide in the point estimate for the regression intercept and slope, and the uncertainty about this parameter estimate (the standard deviation of the parameter distribution in case of the Bayes method and the standard error of the parameter for the classical approach). Furthermore, the standard error in the Bayes summary indicates the computational MCMC error for the estimation of the posterior expected value of the parameters. Given their small values we can say that the numerical precision is high enough.
# The Naive SE is computed as: sqrt(var(x)/length(x)) with x being the concatenated MCMC posterior samples of the three chains. It's the variance normalized by the number of samples, hence it decreases as we lengthen the time series.
# To compare the precision estimate, we take the inverse of the residual standard error squared (ie the sum of squares / (number of observations- number of parameters)). We see also that they are very similar. For the classical analysis, we don't have information about the quality of the estimation of this precision parameter. Also, the true standard deviation parameter (sigma) for the observation error is 4, so the true precision is 0.0625. In both cases, we slightly overestimate this parameter.
# In the Bayesian model output, we see that the true parameters always fall inside the 95% confidence intervals. 
...
# Pass data to list format (with exact covariate names)
...
inits <- list()
for(i in 1:nchain){
 inits[[i]] <- list(beta = rnorm(4,0,5), prec = runif(1,1/200,1/20))
}
...
# The plots indicate that the model converges and we can set the burnin at 300 samples (all of the ratios fall below 1.05 for the first time). 

jags.mat <- as.matrix(jags.out)
pairs(jags.mat)
# The pair plot indicates a strong correlation between the four regression coefficients, but not so strong with the precision.

# Remove burnin
jags.burn <-window(jags.out, start = 300)
jags.burn.mat <- as.matrix(jags.burn)
gelman.plot(jags.burn)
# After removing the burnin samples, we see that the second peak in the variance ratios is much smalles. This is probably because the MCMC chains have reached the value towards which they converge (this may differ substantially from the initial seed).
```
```{r}
summary(jags.burn)
summary(lm(y~x1*x2))
...
beta
# We see that the model does converge to the true values. The uncertainty about the model parameters is very similar to the one estimated by the frequentist linear regression because we have given the Bayes model very uninformative priors about the parameters. Hence, the data has the most weight in the estimation.
```
```{r}
# Plot marginal and parwise joint distributions
plot.marginal <- function(x, x_true, x_name){
  hist(x, breaks=30, main = paste("Histogram of ", x_name), xlab=x_name, freq=F)
  lines(density(x))
  abline(v=x_true, lwd=2,col=2)
}
plot.marginal(jags.burn.mat[,1], beta[1], "beta1") # Slightly overestimated
plot.marginal(jags.burn.mat[,2], beta[2], "beta2")
plot.marginal(jags.burn.mat[,3], beta[3], "beta3")
plot.marginal(jags.burn.mat[,4], beta[4], "beta4")
plot.marginal(jags.burn.mat[,5], 1/sigma^2, "prec")

# All of the parameters are pretty far from the truth. The intercept and the precision are okay, and they're probably the ones least affected by coviarate correlation. The model does not recover the true parameters very well, but neither does the linear regression model. How good are the predictions though? I wonder
```