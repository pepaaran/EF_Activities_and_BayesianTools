...
```{r}
#P# The equivalent code for an implementation with BayesianTools would be:

n.iter = 500                           ## INCREASE THIS NUMBER FOR ACTUAL ANALYSES **************************************

## this code fuses forest inventory data with tree growth data (tree ring or dendrometer band)
## for the same plots. Code is a rewrite of Clark et al 2007 Ecol Appl into BayesianTools

DataFusionLikelihood <- function(param){
  # PSEUDOCODE FOR NOW
  ...
  # Data model DBH
  dnorm(z[i,t], mean=x[i,t], sd=param[1]) # sum?
  
  # Data model tree rings
  dnorm(y[i,t], mean=x[i,t]-x[i, t-1], sd=param[2])
  
  # Process model
  Dnew[i,t] <- x[i, t-1] + param[3]
  dnorm(x)
}
